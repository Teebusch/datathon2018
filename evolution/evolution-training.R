...
df3 %>%
  ...
  ggsave("evolution/img/evolution-d3/Time vs. Power.png")
...
df3 %>%
  ...
  ggsave("evolution/img/evolution-d3/Time vs. Latitude.png")
...
df3 %>%
  filter(bin_min > 345) %>%
  ggplot(aes(x = bin_min, y = power, group = id)) +
  geom_point(size=0.3) +
  scale_x_continuous(breaks = seq(340, 380, 1)) + 
  facet_wrap(  ~ id) + 
  ggtitle("Time vs. Power (Training)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  geom_vline(xintercept = 351.5, color= "red") + 
  ggsave("evolution/img/evolution-d3/Time vs. Power (Race).png")
...
df3_progress_selection = merge(df3_progress, df3_selection_bin) %>% 
  group_by(id) %>% 
  arrange(bin_half) %>% 
  ungroup() %>%
  mutate(bin_half = bin_half / 2 )
# ----------------------------------------------------------

# Speed
df3_progress_selection %>%
  ggplot(aes(x = bin_half, y = order, group = check, color = speed )) +
  geom_line() +
  scale_color_viridis(NULL, option = "C") +
  scale_y_reverse(breaks = seq(1:7), labels = c("1st", "2nd", "3rd", "4th", "5th", "6th", "7th")) + 
  scale_x_continuous(breaks = seq(0, 352, 5) ) +
  xlab("Time (minutes)") +
  ylab("") + 
  theme_bw() +
  facet_grid(idp ~ .) +
  ...
  ggtitle("Evolution of the team by Speed (2018-01-06 UTC) [Training]") +
  ggsave("evolution/img/evolution-d3/Evolution of the team by Speed.png",
         width = 5.79 * 2, height = 4.35 * 2, unit = "in")
...
df3_progress_selection %>%
  ...
  ggtitle("Evolution of the team by Power (2018-01-06 UTC) [Training]") +
  ggsave("evolution/img/evolution-d3/Evolution of the team by Power.png",
         width = 5.79 * 2, height = 4.35 * 2, unit = "in")
...
df3_progress_selection %>%
  ...
  ggtitle("Evolution of the team by Cadence (2018-01-06 UTC) [Training]") +
  ggsave("evolution/img/evolution-d3/Evolution of the team by Cadence.png",
         width = 5.79 * 2, height = 4.35 * 2, unit = "in")
...
df3_progress_selection %>%
  ...
  ggtitle("Evolution of the team by Heart Rate (2018-01-06 UTC) [Training]") +
  ggsave("evolution/img/evolution-d3/Evolution of the team by Heart Rate.png",
         width = 5.79 * 2, height = 4.35 * 2, unit = "in")

# ----------------------------
#  Last part of the race

d3_final = df3_progress_selection %>%
  filter(bin_half > 320)
...
d3_final %>%
  ...
  scale_color_viridis(NULL, option = "C") +
  scale_y_reverse(breaks = seq(1:7), labels = c("1st", "2nd", "3rd", "4th", "5th", "6th", "7th")) + 
  scale_x_continuous(breaks = seq(0, 352, 1) ) +
  xlab("Time (minutes)") +
  ylab("") + 
  theme_bw() +
  facet_grid(idp ~ .) +
  ...
  guides(fill=guide_legend(title="Speed")) +
  ggtitle("Evolution of the team by Speed (2018-01-06 UTC) [Training]") +
  ggsave("evolution/img/evolution-d3/Evolution of the team by Speed - final.png",
         width = 5.79 * 2, height = 4.35 * 2, unit = "in")
...
d3_final %>%
  ...
  guides(fill=guide_legend(title="Power")) +
  ggtitle("Evolution of the team by Power (2018-01-06 UTC) [Training]") +
  ggsave("evolution/img/evolution-d3/Evolution of the team by Power - final.png",
         width = 5.79 * 2, height = 4.35 * 2, unit = "in")
...
# d3_sprint = d3_final %>% 
#   filter(id == "10") %>% 
#   select(bin_half, order) %>% 
#   rename(ref = order)
# 
# d3_final2 = merge(d3_final , d3_sprint) %>%
#   mutate(new_order = as.factor(ref - order))
#   
# # Speed
# d3_final2 %>%
#   ggplot(aes(x = bin_half, y = new_order, group = check, color = idp )) +
#   geom_line() +
#   #scale_color_viridis(NULL, option = "C") +
#   #scale_y_reverse() + 
#   scale_x_continuous(breaks = seq(0, 352, 5) ) +
...
#   guides(fill=guide_legend(title="Speed")) +
#   ggtitle("Evolution of the team by Speed (2018-01-06 UTC) [Training]") +
#   ggsave("evolution/img/evolution-d3/Evolution of the team by Speed (Relative) - final.png",
#          width = 5.79 * 2, height = 4.35 * 2, unit = "in")
